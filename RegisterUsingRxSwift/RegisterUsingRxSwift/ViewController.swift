...
class ViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    private func setupCombine(){
        ...
        let invalidPublisher = Publishers.CombineLatest4(namePublisher, emailPublisher, passwordPublisher, confirmPasswordPublisher).map { (name, email, password, confirmationPassword)  in
            name && email && password && confirmationPassword
        }
        
        invalidPublisher.sink { (isValid) in
            if isValid {
